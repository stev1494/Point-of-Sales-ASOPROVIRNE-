					...
					<table class="table table-hover display responsive no-wrap" width="100%" id="table">
					    <thead>
					        <tr>
					            ...
					            <th data-priority="2">DESTINO</th>
					            <th data-priority="3" class="text-right"></th>
					        </tr>
					    </thead>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								<label for="txt_destino" class="control-label">Destino</label>
								<input type="text" name="txt_destino" id="txt_destino" class="form-control" required="" maxlength="30">
							</div>
						</div>
...
$(document).ready(function() {
	...
	$('#mdlNuevo').on('shown.bs.modal', function (event) {
		var modal = $(this);
		modal.find('.modal-body #txt_destino').focus();
	});
    ...
    $('#formNuevo').submit(function(e) {
        ...
        $.ajax({
            url: 'modulo/destino.php?metodo=guardar',
            type : "POST",
            dataType : 'json',
            data : $(this).serialize()
        })
	...
	$('#mdlEditar').on('show.bs.modal', function (event) {
		...
		$.ajax({
            url: 'modulo/destino.php?metodo=obtener',
            type : "POST",
            dataType : 'json',
            data : {
            	id: id
		...
		.done(function( data, textStatus, jqXHR ) {
			...
			modal.find('.modal-body #txt_destino').val(data.destino);
			$('#cargando').hide();
			$('#formulario').show();
			modal.find('.modal-body #txt_destino').focus();
		})
    ...
    $('#formEditar').submit(function(e) {
        ...
        $.ajax({
            url: 'modulo/destino.php?metodo=editar',
            type : "POST",
            dataType : 'json',
            data : $(this).serialize()
        })
	...
	function getTable() {
	    return  $('#table')
				    .DataTable({
				        ...
				        "ajax":{
				            "method": "POST",
				            "url": "modulo/destino.php?metodo=listado",
				            "complete": function () {
				            	$('#table-body').show();
				            },
