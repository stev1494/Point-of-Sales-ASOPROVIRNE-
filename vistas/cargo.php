					...
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								...
								<input type="text" name="txt_nombre" id="txt_nombre" class="form-control" required="" maxlength="12">
							</div>
						</div>
						<div class="col-md-12">
							<div class="form-group">
								<label for="txt_descripcion" class="control-label">Descripcion</label>
								<input type="text" name="txt_descripcion" id="txt_descripcion" class="form-control" required="" maxlength="12">
							</div>
						</div>
						<div class="col-md-12">
							<div class="form-group">
								<label for="txt_sueldo" class="control-label">Sueldo</label>
								<input type="number" name="txt_sueldo" id="txt_sueldo" class="form-control" required="" min="0">
							</div>
						</div>
...
$(document).ready(function() {
    ...
    $('#formNuevo').submit(function(e) {
        ...
        $.ajax({
            url: 'modulo/cargo.php?metodo=guardar',
            type : "POST",
            dataType : 'json',
            data : $(this).serialize()
        })
	...
	$('#mdlEditar').on('show.bs.modal', function (event) {
		...
		$.ajax({
            url: 'modulo/cargo.php?metodo=obtener',
            type : "POST",
            dataType : 'json',
            data : {
            	id: id
		...
		.done(function( data, textStatus, jqXHR ) {
			...
			modal.find('.modal-body #txt_descripcion').val(data.descripcion);
			modal.find('.modal-body #txt_sueldo').val(data.sueldo);
			$('#cargando').hide();
			$('#formulario').show();
			modal.find('.modal-body #txt_nombre').focus();
		})
    ...
    $('#formEditar').submit(function(e) {
        ...
        $.ajax({
            url: 'modulo/cargo.php?metodo=editar',
            type : "POST",
            dataType : 'json',
            data : $(this).serialize()
        })
	...
	function getTable() {
	    return  $('#table')
				    .DataTable({
				        ...
				        "ajax":{
				            "method": "POST",
				            "url": "modulo/cargo.php?metodo=listado",
				            "complete": function () {
				            	$('#table-body').show();
				            },
